          ...
          <nav className="flex flex-wrap items-center justify-center gap-x-8 gap-y-2">
            {NAV_LINKS.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="text-sm text-[#1C1410] hover:text-[#C85A2A] transition-colors"
              >
                {link.label}
              </Link>
            ))}
          ...
          <div className="flex items-center gap-2 shrink-0">
            {SOCIAL_LINKS.map((social) => (
              <a
                ...
                className="w-8 h-8 rounded-sm text-white flex items-center justify-center hover:opacity-80 transition-opacity"
              >
                {social.icon}
              </a>
            ))}
